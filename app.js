(() => {
  ...
  function initBridge() {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => reject(userError('เชื่อมต่อ Google Apps Script ไม่สำเร็จ')), CONFIG.REQUEST_TIMEOUT_MS);

      const onMessage = (event) => {
        if (!isTrustedBridgeOrigin(event.origin)) return;
        const data = event.data || {};
        if (data.channel !== 'leave-app-bridge') return;

        if (data.type === 'ready') {
          clearTimeout(timeout);
          state.bridgeReady = true;
          window.removeEventListener('message', onReadyOnly);
          resolve();
        }
      };

      const onReadyOnly = onMessage;
      window.addEventListener('message', onReadyOnly);
  ...
  function bridgeCall(action, payload = {}) {
    if (!state.bridgeReady) return Promise.reject(userError('Backend ยังไม่พร้อมใช้งาน'));
