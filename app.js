(() => {
  ...
  function bindEvents() {
    ...
    $$('input[name="newEmployeeType"]').forEach(el => el.addEventListener('change', () => clearFieldError('newEmployeeType')));
    $('newEmployeeBackBtn').addEventListener('click', backToEmployeeRegistration);

    $('leaveForm').addEventListener('submit', onReviewSubmit);

    // อัปเดตสถานะปุ่มตรวจสอบข้อมูลทุกครั้งที่ข้อมูลฟอร์มเปลี่ยน
    $$('input[name="leaveType"]').forEach(el => {
    ...
    $$('input[name="leaveUnit"]').forEach(el => {
      el.addEventListener('change', () => {
        updateLeaveUnitUi();
        updateReviewButtonVisibility();
      });
    ...
    $('leaveAmount').addEventListener('input', () => {
      clearFieldError('leaveAmount');
      updateReviewButtonVisibility();
    });

    $('leaveFrom').addEventListener('change', () => {
      clearFieldError('leaveFrom');
      updateReviewButtonVisibility();
    });

    $('leaveTo').addEventListener('change', () => {
      clearFieldError('leaveTo');
      updateReviewButtonVisibility();
    });

    $('reason').addEventListener('input', (e) => {
      $('reasonCount').textContent = e.target.value.length;
    ...
    $$('[data-dashboard-view]').forEach(btn => {
      btn.addEventListener('click', () => switchDashboardView(btn.dataset.dashboardView));
    });

    $('dashboardYear').addEventListener('change', () => {
      syncDashboardYear('dashboardYear');
      loadDashboard();
    });

    $('dashboardAllYear').addEventListener('change', () => {
      syncDashboardYear('dashboardAllYear');
      loadDashboard();
    });
  }
...
function buildDashboardYearOptions() {
  const currentYearAd = new Date().getFullYear();
  const firstYearAd = 2026; // พ.ศ. 2569
  const lastYearAd = Math.max(firstYearAd, currentYearAd + 1);
  const selectedYearAd = Math.max(firstYearAd, currentYearAd);

  ['dashboardYear', 'dashboardAllYear'].forEach(selectId => {
    const select = $(selectId);
    if (!select) return;

    select.innerHTML = '';

    for (let year = firstYearAd; year <= lastYearAd; year += 1) {
      const option = document.createElement('option');
      option.value = String(year);
      option.textContent = String(year + 543);
      option.selected = year === selectedYearAd;
      select.appendChild(option);
    }
  });
}

function syncDashboardYear(sourceId) {
  ...
  async function loadRecipients() {
    ...
    $$('input[name="recipients"]').forEach(el => {
      el.addEventListener('change', () => {
...
function backToEmployeeRegistration() {
  ...
  clearFieldError('newEmployeeType');
  ...
  function showNewEmployeeRegistration(employeeId) {
    ...
    $$('input[name="newEmployeeType"]').forEach(el => { el.checked = false; });
    clearFieldError('newEmployeeFullName');
    clearFieldError('newEmployeeType');
  ...
  async function createNewEmployee(event) {
    ...
    const employeeId = normalizeEmployeeId(
      state.pendingEmployeeId || $('newEmployeeId').value
    );
    ...
    // อ่านค่าจาก radio ที่ผู้ใช้เลือก: รายวัน / รายเดือน
    ...
    if (!employeeId) {
      showToast(
        'ไม่พบรหัสพนักงานจากขั้นตอนก่อนหน้า กรุณากลับเข้า LIFF ใหม่',
        true
      );
      return;
    }
    ...
    try {
      // สำคัญ: ต้องส่ง employeeType ไปยัง Code.gs ด้วย
      ...
      showToast(
        `เพิ่มข้อมูลสำเร็จ สวัสดีคุณ ${state.employee.fullName}`
      );
      ...
      const message =
        error.userMessage || 'ไม่สามารถเพิ่มข้อมูลพนักงานได้';

      if (
        error.code === 'EMPLOYEE_TYPE_REQUIRED' ||
        error.code === 'EMPLOYEE_TYPE_INVALID' ||
        error.code === 'EMPLOYEE_TYPE_MISSING'
      ) {
        setFieldError('newEmployeeType', message);
      } else if (
        error.code === 'EMPLOYEE_NAME_REQUIRED' ||
        error.code === 'EMPLOYEE_NAME_TOO_LONG' ||
        error.code === 'EMPLOYEE_NAME_MISSING'
      ) {
        setFieldError('newEmployeeFullName', message);
      } else {
  ...
  // Dashboard - ข้อมูลของผู้ใช้งานปัจจุบัน
  if ($('dashboardMyNameTop')) {
    $('dashboardMyNameTop').textContent = employee.fullName || '-';
  }

  if ($('dashboardMyName')) {
    $('dashboardMyName').textContent = employee.fullName || '-';
  }

  if ($('dashboardMyEmployeeId')) {
    $('dashboardMyEmployeeId').textContent = employee.employeeId || '-';
  }

  if ($('dashboardMyDepartment')) {
    $('dashboardMyDepartment').textContent =
      employee.department || CONFIG.DEPARTMENT || '-';
  }

  if ($('dashboardMyAvatar')) {
    $('dashboardMyAvatar').src =
      employee.imageUrl || placeholderAvatar(employee.fullName);
  }
}
  ...
  function updateReviewButtonVisibility() {
    const wrap = $('reviewSubmitWrap');
    if (!wrap) return;

    const leaveType = selectedValue('leaveType');
    const leaveUnit = selectedValue('leaveUnit');
    const leaveAmountText = String($('leaveAmount')?.value || '').trim();
    const leaveAmount = Number(leaveAmountText);
    const leaveFrom = $('leaveFrom')?.value || '';
    const leaveTo = $('leaveTo')?.value || '';
    const reason = String($('reason')?.value || '').trim();
    const recipientCount = $$('input[name="recipients"]:checked').length;

    let amountValid =
      leaveAmountText !== '' &&
      Number.isFinite(leaveAmount) &&
      leaveAmount > 0;

    if (amountValid && leaveUnit === 'HOUR') {
      amountValid = Number.isInteger(leaveAmount * 2);
    }

    if (amountValid && leaveUnit === 'DAY') {
      amountValid = Number.isInteger(leaveAmount);
    }
    ...
    wrap.classList.toggle('hidden', !formReady);
  }
...
async function sendFlex(flexMessage) {
  ...
  async function loadDashboard() {
    ...
    const activeYearSelect = state.dashboardView === 'all'
      ? $('dashboardAllYear')
      : $('dashboardYear');

    const selectedYear = Number(activeYearSelect?.value || currentYear);
    syncDashboardYear(state.dashboardView === 'all' ? 'dashboardAllYear' : 'dashboardYear');
    ...
    try {
      ...
      if (state.activeTab === 'dashboard') {
        showToast(
          error.userMessage || 'โหลด Dashboard ไม่สำเร็จ',
          true
        );
      }
    }
  ...
  function renderDashboard(data) {
    ...
    const selectedYearAd = Number(
      (state.dashboardView === 'all' ? $('dashboardAllYear')?.value : $('dashboardYear')?.value) ||
      $('dashboardYear')?.value ||
      new Date().getFullYear()
    );

    const responseYearAd = Number(data.year);
    const safeYearAd = Number.isFinite(responseYearAd) && responseYearAd >= 2026
      ? responseYearAd
      : selectedYearAd;

    const responseYearBe = Number(data.yearBe);
    const yearBe = Number.isFinite(responseYearBe) && responseYearBe >= 2569
      ? responseYearBe
      : safeYearAd + 543;

    if ($('statMyRank')) {
      $('statMyRank').textContent =
        data.mySummary?.rank ? String(data.mySummary.rank) : '-';
    }

    if ($('mySummaryPeriod')) {
      $('mySummaryPeriod').textContent =
        `ปี ${yearBe} · ${data.hoursPerDay || state.employee?.hoursPerDay || '-'} ชั่วโมง = 1 วัน`;
    }

    if ($('mySummaryCards')) {
      $('mySummaryCards').innerHTML =
        order.map(type => `
          <div class="summary-row">
            <span>${leaveTypeLabel(type)}</span>
            <strong>${escapeHtml(summary[type]?.display || '0 วัน 0 ชั่วโมง')}</strong>
          </div>
        `).join('') +
        `
          <div class="summary-row dashboard-summary-total">
            <span>รวมทั้งหมด</span>
            <strong>${escapeHtml(data.mySummary?.totalDisplay || '0 วัน 0 ชั่วโมง')}</strong>
          </div>
        `;
    }

    if ($('dashboardAllPeriod')) {
      $('dashboardAllPeriod').textContent =
        `ปี ${yearBe} · สมาชิกทั้งหมด ${Number(data.stats?.employeeCount || 0)} คน · ไม่รวมลาพักร้อน`;
    }
    ...
    if ($('rankingList')) {
      ...
      $('rankingList').innerHTML = ranking.length
        ? ranking.map(item => `
          <div class="rank-row dashboard-member-row">
            <div class="rank-number">${escapeHtml(String(item.rank ?? '-'))}</div>

            <img
              class="rank-avatar dashboard-member-avatar"
              src="${escapeAttr(item.imageUrl || placeholderAvatar(item.fullName))}"
              alt=""
            >

            <div class="rank-person dashboard-member-person">
              <strong>${escapeHtml(item.fullName || '-')}</strong>
              <small>รหัส ${escapeHtml(item.employeeId || '-')}</small>
            </div>

            <div class="dashboard-rank-total-card">
              <span>ลาทั้งหมดไม่รวมพักร้อน</span>
              <strong>${escapeHtml(item.rankDisplay || '0 วัน 0 ชั่วโมง')}</strong>
            </div>
          </div>
        `).join('')
        : '<p class="muted dashboard-empty">ไม่พบสมาชิกในระบบ</p>';
    }
  }

  function switchDashboardView(viewName) {
    const view = viewName === 'all' ? 'all' : 'me';
    ...
    $$('[data-dashboard-view]').forEach(btn => {
      btn.classList.toggle(
        'active',
        btn.dataset.dashboardView === view
      );
    });

    if ($('dashboardMePanel')) {
      $('dashboardMePanel').classList.toggle(
        'active',
        view === 'me'
      );
    }

    if ($('dashboardAllPanel')) {
      $('dashboardAllPanel').classList.toggle(
        'active',
        view === 'all'
      );
    }
  ...
  function switchTab(tabName) {
    ...
    if (tabName === 'dashboard') {
      // ทุกครั้งที่กด Dashboard ให้เริ่มที่หน้า "เฉพาะคุณ"
      switchDashboardView('me');
      loadDashboard();
    }
